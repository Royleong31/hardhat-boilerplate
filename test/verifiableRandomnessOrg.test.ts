...
describe("VerifiableRandomnessOrg", async () => {
  ...
  beforeEach(async () => {
    ...
    verifiableRandomnessOrg = await ethers.getContractAt(
      "VerifiableRandomnessOrg",
      VRODeployment.address,
      accounts[0]
    );
  ...
  it("Should allow people to get a random number", async () => {
    if (runTest) {
      await run("fund-link", {
        contract: verifiableRandomnessOrg.address,
        linkaddress: LinkToken.address,
      }); // we do this also to move the blocks along

      let range = 77;
      let randomNumber = 777;
      let transactionResponse = await verifiableRandomnessOrg.getRandomNumber(
        range
      );
      ...
      await vrfCoordinator.callBackWithRandomness(
        requestId,
        randomNumber,
        verifiableRandomnessOrg.address
      );
      expect(
        await verifiableRandomnessOrg.s_requestIdToRNG(requestId)
      ).to.equal(randomNumber % range);
    }
  });
});
